...
impl Plot {
    ...
    fn count_ways(&self, x: usize, y: usize) -> u32 {
            ...
            if x + 3 < self.items[y].len() && y >= 3 {
                if (0..=3).all(|i| self.items.get(y.saturating_sub(i)).and_then(|row| row.get(x + i)).map_or(false, |&c| c == "XMAS".chars().nth(i).unwrap())) {
                    count += 1;
                }
            }
            ...
            if x >= 3 && y >= 3 {
                if (0..=3).all(|i| self.items.get(y.saturating_sub(i)).and_then(|row| row.get(x.saturating_sub(i))).map_or(false, |&c| c == "XMAS".chars().nth(i).unwrap())) {
                    count += 1;
                }
            }

            // Check diagonal down right
            if x + 3 < self.items[y].len() && y + 3 < self.items.len() {
                if (0..=3).all(|i| self.items.get(y + i).and_then(|row| row.get(x + i)).map_or(false, |&c| c == "XMAS".chars().nth(i).unwrap())) {
                    count += 1;
                }
            }

            // Check diagonal down left
            if x >= 3 && y + 3 < self.items.len() {
                if (0..=3).all(|i| self.items.get(y + i).and_then(|row| row.get(x.saturating_sub(i))).map_or(false, |&c| c == "XMAS".chars().nth(i).unwrap())) {
                    count += 1;
                }
            }
