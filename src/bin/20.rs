...
pub fn part_one(input: &str) -> Option<u32> {
    ...
    for y in 0..costs.len() {
        for x in 0..costs[0].len() {
            if costs[y][x] != 0 {
                moves_saved.extend(bfs_with_cap(&costs, (x, y), 2).unwrap());
            }
        }
    }
...
pub fn part_two(input: &str) -> Option<u32> {
    ...
    for y in 0..costs.len() {
        for x in 0..costs[0].len() {
            if costs[y][x] != 0 {
                moves_saved.extend(bfs_with_cap(&costs, (x, y), 20).unwrap());
            }
        }
    }
    moves_saved.sort();
